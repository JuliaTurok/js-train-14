...
const inputDate = date.toISOString();
  ...
  const lastDayOfMonth = new Date(nextMonthDate.getTime() - 86400000); // 86400000 мілісекунд в одному дні.
...
function getAge(birthDate) {
  ...
  // Повертаємо об'єкт з датою народження та обчисленим віком.
   if (!(birthDate.getTime)) {
    ...
    return {birthDate: birthDate.toISOString(), years} ;
...
function compareDates(date1, date2) {
 ...
 const comparison = (date1 < date2) ? -1 : (date1 === date2) ? 0 : 1
...
function getDaysDifference(startDate, endDate) {
  ...
  // Повертаємо об'єкт з початковою та кінцевою датами та різницею в днях.
   ...
   const comparison = endTime - startTime;
   const daysDifference = Math.floor(comparison / (1000 * 60 * 60 * 24));
